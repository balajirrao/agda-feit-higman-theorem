...
module Lemma2-4-core where
  ...
  .neck-e#e₁ : ∀ {e} → (nck : Neck e) → (pt e) # (ln (neck-e₁ nck))
  neck-e#e₁ nck =  pf (proj₁ nck)

  .neck-e₁#e₂ : ∀ {e} → (nck : Neck e) → (ln (neck-e₁ nck)) # (pt (neck-e₂ nck))
  neck-e₁#e₂ nck =  pf (proj₂ nck)

  neck-rev : ∀ {e} → (nck : Neck e) → Neck (neck-e₂ nck)
  neck-rev {e} (e₁ ∶ e#e₁ , e₂ ∶ e₁#e₂) = (e₁ ∶ (#sym e₁#e₂)) , e ∶ #sym e#e₁

  data ppchain : P → P → Set where
    [_] : (e : P) → ppchain e e
    _∷_ : ∀ {e f} (nck : Neck e) → (c : ppchain (neck-e₂ nck) f) → ppchain e f
 
  Xdec : (x : X) → (Σ P (λ p → (pt p) ≡ x)) ⊎ (Σ L (λ l → (ln l) ≡ x))
  Xdec (pt x) = inj₁ (x , refl)
  Xdec (ln x) = inj₂ (x , refl)
 ...
 -- _as-ppc c = {!!}
  ...
  _as-ppc : ∀ {e f} (c : chain (pt e) (pt f)) → ppchain e f
  _as-ppc {.f} {f} [ .(pt f) ] = [ f ]
  _as-ppc {e} {f} (_∷_ .(pt e) {{e<>f}} {{e#f}} [ .(pt f) ]) =
                                                  ⊥-elim (IP-pt e<>f e#f)
  _as-ppc {e} (_∷_ {f₁} .(pt e) {{e<>f}} {{e#f}}
                        (_∷_ {e₁} .f₁ {{e<>f₁}} {{e#f₁}} c)) with e₁ | f₁
  ... | pt x | pt x₁ = ⊥-elim (IP-pt e<>f e#f)
  ... | ln x | pt x₁ = ⊥-elim (IP-pt e<>f e#f)
  ... | ln x | ln x₁ = ⊥-elim (IP-pt e<>f₁ e#f₁)
  ... | pt x | ln x₁ = ((x₁ ∶ e#f) , x ∶ e#f₁) ∷ (c as-ppc)

  -- View a ppchain as a chain
  _as-c : ∀ {e f} (ppc : ppchain e f) → chain (pt e) (pt f)
  [ e ] as-c = [ pt e ]
  _as-c {e} {f} ((e₁ ∶ _ , e₂ ∶ _) ∷ ppc) =
                     _∷_ (pt e) {{λ ()}} (_∷_ (ln e₁) {{λ ()}} (ppc as-c))
  
 -- as-ppc ∘ as-c ≡ id
  lem-id₀ : ∀ {e f} (c : chain (pt e) (pt f)) → (c as-ppc) as-c ≡ c
  lem-id₀ {.f} {f} ([ .(pt f) ]) = refl
  lem-id₀ {e} {f} (_∷_ .(pt e) {{e<>f}} {{e#f}} [ .(pt f) ]) =
                                                  ⊥-elim (IP-pt e<>f e#f)
  lem-id₀ {e} {f} (_∷_ .(pt e) {{e<>f}} {{e#f}}
                            (_∷_ {e₁} f₁ {{e<>f₁}} {{e#f₁}} c))
    with e₁ | f₁
  ... | pt x | pt x₁ = ⊥-elim (IP-pt e<>f e#f)
  ... | pt x | ln x₁ rewrite (lem-id₀ c) = refl
  ... | ln x | pt x₁ = ⊥-elim (IP-pt e<>f e#f)
  ... | ln x | ln x₁ = ⊥-elim (IP-pt e<>f₁ e#f₁)
  ...
  lem-id₁ [ e ] = refl
  lem-id₁ (nck ∷ ppc) rewrite lem-id₁ ppc = refl
  ...
  lem-2xρ-len {ppc = (nck ∷ ppc)} rewrite lem-2xρ-len {ppc = ppc} =
                     solve 1
                           (λ x →
                              con 2 :+ x :+ (x :+ con 0) :=
                                  con 1 :+ x :+
  ...
  lem-len/2-ρ {ppc = (nck ∷ ppc)} = cong suc lem-len/2-ρ
  ...
  ... | a rewrite lem-id₀ (sc (pt e) (pt f)) = a
  ...
  lem-ρ'≥ρ : ∀ {e f} {ppc : ppchain e f} →
                     True (1 ≤? ρ e f) → True (1 ≤? ρ' ppc)
  lem-ρ'≥ρ {e} {f} {ppc} ≥1 = fromWitness
                              (begin
                                1 ≤⟨ toWitness ≥1 ⟩
                                ρ e f ≤⟨ sppc-ρ-shorter-than ppc ⟩
                                (ρ' ppc ∎))
  ...
  tailpp : ∀ {e f} → (ppc : ppchain e f) →
             {≥1 : True (1 ≤? ρ' ppc)} →
             ppchain (neck-e₂ (ppneck  ppc {≥1 = ≥1} )) f
  tailpp {.f} {f} [ .f ] {()}
  tailpp (nck ∷ ppc) = ppc

  lem-tailpp-ρ : ∀ {e f} {ppc : ppchain e f} {≥1 : True (1 ≤? ρ' ppc)} →
                   ρ' (tailpp ppc  {≥1 = ≥1}) ≡ pred (ρ' ppc)
  lem-tailpp-ρ {.f} {f} {[ .f ]} {()}
  lem-tailpp-ρ {ppc = (nck ∷ ppc)} = refl


  module ρ-shortest  where
    lem-neckp : {e f : P} (ppc : ppchain e f) {≥1 : True (1 ≤? ρ' ppc)} →
                pt (neck-e₂ (ppneck ppc  {≥1 = ≥1})) ≡ neck (tail (ppc as-c))
    lem-neckp {.f} {f} [ .f ] {()}
    lem-neckp (nck ∷ ppc) = refl

    lem-tailpp : ∀ {e f} (ppc : ppchain e f) {≥1 : True (1 ≤? ρ' ppc)} →
                         len ((tailpp ppc  {≥1 = ≥1}) as-c) ≡
                                               len (tail (tail (ppc as-c)))
    lem-tailpp {.f} {f} [ .f ] {()}
    lem-tailpp (nck ∷ ppc) = refl

    tailpp-ρ-shortest : ∀ {e f} {ppc : ppchain e f} →
                                {≥1 : True (1 ≤? ρ' ppc)} →
                          ppc is-ρ-shortest → tailpp ppc is-ρ-shortest
    tailpp-ρ-shortest {e} {f} {ppc} {≥1} cis =
                          helper₂ (
                            begin (
                              2 * ρ' (tailpp ppc)
                                ≡⟨ sym lem-2xρ-len ⟩ 
      ...
      where open ≡-Reasoning
            ...
            helper₁ : len (sc (neck (tail (ppc as-c))) (pt f)) ≡
                              len (sc (pt (neck-e₂ (ppneck ppc))) (pt f))
            helper₁ rewrite sym (lem-neckp ppc) = refl
  ...
  sppc-irred {e} {f} rewrite lem-id₀ (sc (pt e) (pt f)) =
  ...
  lem-ρ-len<n : ∀ {e f} {ppc : ppchain e f} →
                  ρ' ppc < ⌈ n /2⌉ → len (ppc as-c) < n
  ...
  lem-len-ρ<⌈n/2⌉ : ∀ {e f} {ppc : ppchain e f} →
                            len (ppc as-c) < n → ρ' ppc < ⌈ n /2⌉
  ...
  A₁'-ρ {e} {f} | a | b = begin ρ' (sppc e f)
                                ≤⟨ a ⟩ ρ' (proj₁ (A₁-ρ e f))
                                ≤⟨ b ⟩ ⌊ n /2⌋  ∎
        where open Data.Nat.≤-Reasoning
                                
  -- Axiom A₂ in ρ terms, but now with proof
  A₂-ρ : ∀ {e f} (ppc ppc' : Σ' (ppchain e f)
                 (λ z → ρ' z < ⌈ n /2⌉ × irred (z as-c))) → ppc ≡ ppc'
  A₂-ρ (ppc ∶ p) (ppc' ∶ p')
       with cong (λ x → el x as-ppc)
            (A₂ (ppc as-c ∶ ((lem-ρ-len<n (proj₁ p) , proj₂ p)))
                              (ppc' as-c ∶ ((lem-ρ-len<n (proj₁ p') , proj₂ p'))))
  ... | z rewrite lem-id₁ ppc | lem-id₁ ppc' | z = refl
   
  lem-neckl : {x y : P} (ppc : ppchain x y) {≥1 : True (1 ≤? ρ' ppc)} →
                             ln (neck-e₁ (ppneck ppc)) ≡  (neck (ppc as-c))
  lem-neckl [ e ] = λ {}
  lem-neckl (nck ∷ ppc) = refl
   
  neck! : ∀ {e} {nck nck' : Neck e} → e ≢ neck-e₂ nck →
                                      neck-e₂ nck ≡ neck-e₂ nck' → nck ≡ nck'
  neck! {e} {e₁ ∶ e#e₁ , e₂ ∶ e₁#e₂} {e₁' ∶ e₁'#e , .e₂ ∶ e₁'#e₂'} neq refl =
    ...
    where c : chain (pt e) (pt e₂)
          ...
          irredc : irred c
          irredc {zero} x = IP-pt ( (λ eq → neq (pt-inj eq))) x 
          irredc {suc n} {()} x
          ...
          e₁≡e₁' : ln e₁ ≡ ln e₁'
          e₁≡e₁' = cong neck (cong el (
                        A₂ (c ∶ ((s≤s (s≤s (s≤s z≤n))) , irredc))
                                (c' ∶ ((s≤s (s≤s (s≤s z≤n))) , irredc'))))
