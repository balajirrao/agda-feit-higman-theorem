...
open import Data.Nat hiding (_≟_; compare) renaming
     ( _+_ to _N+_; _∸_ to _N∸_
       ; _≤_ to _N≤_; _≥_ to _N≥_;
       _<_ to _N<_; _≤?_ to _N≤?_; pred to Npred)
...
module FinBijections where
  ...
  sum-bij {a} =
    ...
    where to : ∀ {a b} → Fin a ⊎ Fin b → Fin (a N+ b)
          ...
          reduce-raise : ∀ {a b} → (y : Fin b)
                           (p : toℕ (raise a y) N≥ a) →
                            reduce≥ (raise a y) p ≡ y
          reduce-raise {zero} y p = refl
          reduce-raise {suc a} y (s≤s p) = reduce-raise y p

          left-inverse-of :  ∀ {a b} (x : Fin a ⊎ Fin b) →
                                  from {a} {b} (to x) ≡ x
          left-inverse-of {zero} (inj₁ ())
          left-inverse-of {zero} (inj₂ y) = refl
          left-inverse-of {suc a} {b} (inj₁ x)
            with toℕ (inject≤ x (s≤s (m≤m+n a b))) N≤? a
          ... | yes p rewrite inject≤-lemma x (m≤m+n (suc a) b) =
                              PropEq.cong inj₁ (fromℕ≤-toℕ x (s≤s p))
          ... | no ¬p = ⊥-elim (¬p
                        (begin toℕ (inject≤ x (s≤s (m≤m+n a b)))
                                   ≡⟨ inject≤-lemma x (m≤m+n (suc a) b) ⟩
                               toℕ x ≤⟨ prop-toℕ-≤ x ⟩ (a ∎)))
          left-inverse-of {suc a} (inj₂ y) with suc (toℕ (raise a y)) N≤? a
          ... | yes p  rewrite (toℕ-raise a y) = ⊥-elim (helper {a} {toℕ y} p)
            where helper : ∀ {i j} → 1 N+ i N+ j N≤ i → ⊥
                  helper {zero} ()
                  helper {suc i} (s≤s x) = ⊥-elim (helper x)
          ... | no ¬p with ≰⇒> ¬p
          ... | s≤s k = PropEq.cong inj₂ (reduce-raise y k)
    
  sum′ : ∀ {n} → Vec ℕ n → ℕ
  sum′ {zero} [] = zero
  sum′ {suc n} (x ∷ v) = x N+ sum′ v

  bij-split : ∀ {a} → (g : Fin (suc a) → ℕ) → (Fin (g zero) ⊎
                      Σ (Fin a) (λ x → Fin (g (suc x)))) ↔
                      Σ (Fin (suc a)) (λ x → Fin (g x))
  bij-split {a} g =
    record { to = record { _⟨$⟩_ = from ; cong = PropEq.cong _ } ;
             from = record { _⟨$⟩_ = to ; cong = PropEq.cong _ } ;
             inverse-of = record { left-inverse-of = right-inverse-of ;
             right-inverse-of = left-inverse-of } }
    where to : Σ (Fin (suc a)) (λ x → Fin (g x)) →
               Fin (g (zero)) ⊎ Σ (Fin a) (λ x → Fin (g (raise 1 x)))
          to (zero , proj₂) = inj₁ proj₂
          to (suc proj₁ , proj₂) = inj₂ (proj₁ , proj₂)

          from : Fin (g (zero)) ⊎ Σ (Fin a) (λ x → Fin (g (suc x))) →
                                         Σ (Fin (suc a)) (λ x → Fin (g x))
  ...
  vec-bij-split : ∀ {a} (g : Fin (suc a) → ℕ) →
                        Fin (sum′ {suc a} (tabulate g)) ↔
                        (Fin (g zero) ⊎ Σ (Fin a) (λ x → Fin (g (suc x))) )
  vec-bij-split {zero} g rewrite (+-comm {g zero} {zero}) =
    record { to = record { _⟨$⟩_ = to ; cong = PropEq.cong _ } ;
             from = record { _⟨$⟩_ = from ; cong = PropEq.cong _ } ;
    ...
    where to : Fin (g zero) →
          ...
          from : Fin (g zero) ⊎
                     (Σ (Fin zero) (λ x → Fin (g (suc x)))) → Fin (g zero)
          from (inj₂ (() , proj₂))
          from (inj₁ y) = y
  ...
  vec-bij-split {suc a} g = bij
    where open import Relation.Binary.Sum
          bij : Fin ((g zero) N+
                (sum′ {suc a} (tabulate (λ x → g (suc x))))) ↔
                  (Fin (g zero) ⊎ Σ (Fin (suc a)) (λ x → Fin (g (suc x))) )
          bij = Iid ⊎-↔ (bij-split (λ x → g (suc x)) ∘
                        vec-bij-split {a} (λ x → g (suc x))) ∘
                 Isym (sum-bij {g zero} {sum′ {suc a}
                               (tabulate (λ x → g (suc x)))})
             
      
  vec-bij : ∀ {a} (g : Fin (suc a) → ℕ) → Fin (sum′ {suc a} (tabulate g)) ↔
                                        Σ (Fin (suc a)) (λ x → Fin (g x))
  vec-bij {a} g = bij-split g ∘ vec-bij-split g


  fromℕ-bij : {P Q : ℕ → Set} {a : ℕ} → (∀ {x} → P x ↔ Q x) → ({y : Fin a} →
                                                     P (toℕ y) ↔ Q (toℕ y))
  ...
  prod-bij : ∀ {a b} → (Fin a × Fin b) ↔ (Fin (a * b))
  ...
  fin-exclude₀ : ∀ {a} → (Σ (Fin (suc a)) (λ x₁ → x₁ ≢ zero)) ↔ (Fin a)
  fin-exclude₀ {a} = record
                     { to = record { _⟨$⟩_ = to ; cong = PropEq.cong to }
                     ; from = record { _⟨$⟩_ = from ; cong = PropEq.cong from }
                     ; inverse-of =
                       record { left-inverse-of = left-inverse-of ;
  ...
  tozero-bij {suc a} x =
    record { to = record { _⟨$⟩_ = to ; cong = PropEq.cong to } ;
             from = record { _⟨$⟩_ = to ; cong = PropEq.cong to } ;
             inverse-of = record { left-inverse-of = inverse-of ;
             right-inverse-of = inverse-of } }
    where to = tozero x
  ...
  fin-exclude′ : ∀ {a} → (x : Fin (suc a)) →
                   (Σ (Fin (suc a)) (λ x₁ → x₁ ≢ x)) ↔
                     (Σ (Fin (suc a)) (λ x₁ → x₁ ≢ zero))
  fin-exclude′ x =
    Σ↔ (tozero-bij x) (λ {y} →
    record { to = record { _⟨$⟩_ = λ x₁ x₂ → x₁
                (sym $ Inverse.injective (tozero-bij x)
               (trans (lem→zero {x = x}) (sym x₂))) ; cong = PropEq.cong _ } ;
             from = record { _⟨$⟩_ = λ x₁ x₂ → x₁
               (subst (λ z → tozero z y ≡ zero) x₂ (lem→zero {x = y})) ;
                      cong = PropEq.cong _ } })
               where postulate Σ↔ : predicate-irrelevant-Σ↔

  fin-exclude : ∀ {a} → ( x : Fin (suc a)) →
                        (Σ (Fin (suc a)) (λ x₁ → x₁ ≢ x)) ↔ (Fin a)
  fin-exclude {a} x = fin-exclude₀ ∘ (fin-exclude′ x)

  fin-exclude₂′ : ∀ {a} (x y : Fin (suc (suc a))) →
                        (Σ (Fin (suc (suc a))) (λ x₁ → x₁ ≢ x × x₁ ≢ y)) ↔
                          Σ (Σ (Fin (suc (suc a))) (λ x₁ → x₁ ≢ x))
                                                      (λ x₁ → proj₁ x₁ ≢ y)
  fin-exclude₂′ x y =
  ...
  fin-exclude₂″ : ∀ {a} → (x y : Fin (suc (suc a))) → x ≢ y →
                    Σ (Σ (Fin (suc (suc a))) (λ x₁ → x₁ ≢ x))
                                   (λ x₁ → proj₁ x₁ ≢ y) ↔ Fin a
  fin-exclude₂″  {a} x y neq =
    fin-exclude {a} (Inverse.to (fin-exclude x) ⟨$⟩ (y , ≢sym neq)) ∘
                    Σ↔ (fin-exclude {suc a} x)
                      (record { to = record { _⟨$⟩_ = to ;
                                              cong = PropEq.cong to } ;
                                from = record { _⟨$⟩_ = from ;
                                              cong = PropEq.cong from } })
    where postulate Σ↔ : predicate-irrelevant-Σ↔
          to : {x₁ : Σ (Fin (suc (suc a))) (λ x₂ → x₂ ≢ x)} →
               proj₁ x₁ ≢ y →
               Inverse.to (fin-exclude x) ⟨$⟩ x₁ ≢
                 Inverse.to (fin-exclude x) ⟨$⟩ (y , (≢sym neq))
          to {x₁} neq₀ = λ eq → neq₀
                  (PropEq.cong proj₁ $ Inverse.injective (fin-exclude x) eq)

          from : {x₁ : Σ (Fin (suc (suc a))) (λ x₂ → x₂ ≢ x)} →
                         Inverse.to (fin-exclude x) ⟨$⟩ x₁ ≢
                           Inverse.to (fin-exclude x) ⟨$⟩ (y , (≢sym neq)) →
                                                               proj₁ x₁ ≢ y
          from {x₁} neq₀ = λ eq → neq₀ (PropEq.cong
                             (_⟨$⟩_ (Inverse.to (fin-exclude x)))
                                    (Inverse.to Σ-≡,≡↔≡ ⟨$⟩ (eq , ext)))
             where postulate ext : {A : Set} {a b : A} {x y : a ≢ b} → x ≡ y

  fin-exclude₂ :  ∀ {a} (x y : Fin (suc (suc a))) →
               x ≢ y → (Σ (Fin (suc (suc a))) (λ x₁ → x₁ ≢ x × x₁ ≢ y)) ↔ Fin a
  ...
  fin-one {A} x = record
    ...
    where to : Σ A (λ x₁ → x₁ ≡ x) → Fin 1
          ...
          left-inverse-of : ∀ x → from (to x) ≡ x
          left-inverse-of y = Inverse.to Σ-≡,≡↔≡ ⟨$⟩ ((sym $ proj₂ y) ,
